...
void projectVector(struct Args_device *d_args, int** indexesX, float **newValuesX,int seedX)
{
	int currentIndex;
	int currentSign;
	...
	for(int j = 1; j<=d_args->useInst[0];j++)
	{
		...
		if(xKey[0]!=0) 
		{
			...
			{
				currentIndex = getIndex(xKey[i], seedX, d_args->dimXProj);
				currentSign = getSign(xKey[i], seedX);
...
void buildTree(struct Args_device *d_args,int seedX,int seedY,int tree_no,struct Node *root)
{
	int** indexesX = new int*[d_args->useInst[0]];
	float** newValuesX = new float*[d_args->useInst[0]];
	
	projectVector(d_args,indexesX,newValuesX,seedX);
	...
	computeSeparateInstances(indexesX,newValuesX,d_args->useInst,Y_ind,Y_val,seedY,d_args->dimYProj,d_args->dimXProj,d_args->sparsity,childrenInstancesIndex,childrenInstancesValues);
		...
		else
		{
			...
			for(int i=0;i<noChild;i++)
			{
				if(noChildCluster[i]>0)
				{
					...
					computeSeparateInstances(indexesX,newValuesX,childuseInst,Y_ind,Y_val,seedY,d_args->dimYProj,d_args->dimXProj,d_args->sparsity,childchildrenInstancesIndex,childchildrenInstancesValues);
...
int main(int argc,char** argv)
{
	...
	for(int i=0;i<noTrees;i++)
	{
		...
		th1[i] = thread(buildTree,args,seedX[i],seedY[i],tree_no,root[i]);
	...
	for(int i=0;i<noTrees;i++)
	{
		...
		projectVector(T_args,projindexTest,projvalueTest,seedX[i]);
